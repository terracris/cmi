...
def inverse_kinematics(target_position, initial_guess, L1, L2, L3, L4, L5, y_offset):
    start = time.time()
    result = minimize(objective, initial_guess, args=(target_position, L1, L2, L3, L4, L5, y_offset), method='BFGS')
    end = time.time()

    elapsed_time = end - start
    return result.x, result, elapsed_time   # Return the optimized joint angles
...
optimized_thetas, result, elapsed_time = inverse_kinematics(target_position, initial_guess, L1, L2, L3, L4, L5, y_offset)

print(f"Optimized joint angles (radians): {[round(degrees(x), 4) for x in optimized_thetas]}")
